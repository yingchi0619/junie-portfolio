  ...
  useFrame(() => {
    ...
    if (windows.current)
      windows.current.children.forEach((m, i) => {
        ...
        mat.color.set(lit ? '#c39a4f' : targetGlow ? '#bdc897' : '#897c64');
        mat.emissive.set(lit ? GOLD : '#a48c5b');
    ...
    if (beacon.current) {
      ...
      mat.color.set(gold ? GOLD : planned ? '#fff5d9' : ICE);
    }
    if (ring.current) {
  ...
  return (
    <group>
      ...
      <group ref={body} position={[0, 0, 0]}>
        <Box at={[p.x, 0.07, p.z]} size={[1.56, 0.15, 1.56]} color="#c9bda3" />
        <Box
...
function Road({
  ...
  useFrame(() => {
    if (stripe.current) {
      ...
      mat.color.set(game.trafficRed(keyId) ? '#a96c55' : '#8b9b6a');
      mat.opacity = 0.65;
    }
  });
  return (
    <group>
      ...
      >
        ...
        />
        <meshStandardMaterial color="#ac9e81" roughness={0.95} />
      </mesh>
      {[0.2, 0.5, 0.8].map((t, i) => (
        <mesh
          key={i}
        ...
        >
          ...
          <meshBasicMaterial color="#e1d7bf" transparent opacity={0.7} />
        </mesh>
      ))}
      {game.config.traffic.includes(keyId) && (
        ...
        >
          ...
          />
          <meshBasicMaterial color="#a96c55" transparent opacity={0.6} />
        </mesh>
      )}
      {closed && (
        <group
        ...
        >
          <Box at={[0, 0.27, 0]} size={[1, 0.15, 0.16]} color="#b7865f" />
          {[-0.38, 0.38].map((n) => (
            <group key={n}>
              <Box
                at={[n, 0.14, 0]}
              ...
              <mesh position={[n, 0.1, 0.28]}>
                ...
                <meshStandardMaterial color="#b7865f" />
              </mesh>
            </group>
          ))}
        </group>
...
function Route({ game, reduced }: { game: DashGame; reduced: boolean }) {
  const geometry = useMemo(() => {
      ...
      g.setAttribute(
        'position',
        new THREE.BufferAttribute(new Float32Array(6000 * 3), 3).setUsage(
  ...
  useFrame(() => {
    ...
    if (key !== last.current) {
      ...
      if (pipe.current) {
        ...
        pipe.current.geometry =
          samples.length > 1
            ? new THREE.TubeGeometry(
                new THREE.CatmullRomCurve3(
                  samples.map((p) => new THREE.Vector3(p.x, 0.08, p.z)),
                ),
                Math.min(260, samples.length),
                0.033,
                5,
                false,
              )
...
function Truck({ game, reduced }: { game: DashGame; reduced: boolean }) {
  ...
  return (
    <>
      <group ref={ref}>
        <Box at={[0, 0.22, 0]} size={[0.48, 0.3, 0.9]} color="#95a079" />
        <Box at={[0, 0.44, -0.18]} size={[0.46, 0.3, 0.5]} color="#f1e5cd" />
        <Box at={[0, 0.36, 0.3]} size={[0.44, 0.23, 0.28]} color="#899773" />
        <Box at={[0, 0.42, 0.451]} size={[0.34, 0.16, 0.018]} color="#655c4a" />
        <Box
          at={[0, 0.19, 0.47]}
          size={[0.42, 0.06, 0.04]}
          color="#ebc889"
        ...
        {[-0.26, 0.26].flatMap((x) =>
          [-0.29, 0.29].map((z) => (
            ...
            >
              ...
              <meshStandardMaterial color="#514a3d" />
            </mesh>
          )),
        )}
        <Box
  ...
  useFrame(() => {
    group.current?.children.forEach((m, i) => {
      ...
      if (e && age < 0.7) {
        ...
        const mat = (m as THREE.Mesh).material as THREE.MeshStandardMaterial;
        mat.color.set(e.gold ? GOLD : ['#ffd28a', '#a5b38b', '#e7b4a0'][i % 3]);
...
function Rain({ game, reduced }: { game: DashGame; reduced: boolean }) {
  ...
  return (
    <instancedMesh ref={ref} args={[undefined, undefined, 50]}>
      ...
      <meshBasicMaterial color="#b6bba3" transparent opacity={0.23} />
    </instancedMesh>
  );
}
function World({
  ...
  useFrame((_, dt) => {
    ...
    const cam = camera as THREE.OrthographicCamera;
    const zoom =
      !reduced && game.level === 2
        ? game.phase === 'level'
          ? 1.065
          : game.finalApproach && game.phase === 'driving'
            ? 1.04
            : 1
        : 1;
  ...
  return (
    <>
      <ambientLight intensity={1.15} color="#f4e7cd" />
      <hemisphereLight args={['#f3e8d0', '#a09576', 1.1]} />
      <directionalLight
        position={[-5, 13, 7]}
        color="#ffe4bc"
        intensity={2.2}
      ...
      />
      <directionalLight position={[8, 4, -9]} color="#e4e8cd" intensity={0.8} />
      <fog attach="fog" args={['#ece5d6', 31, 52]} />
      <Box at={[0, -0.25, 0]} size={[16, 0.48, 16]} color="#c5c3a5" />
      <Box at={[0, -0.53, 0]} size={[16.15, 0.08, 16.15]} color="#b1aa89" />
      <group key={key}>
        {ROADS.map((r) => (
          ...
          return (
            <group key={`tree${i}`} position={[x, 0, z]}>
              ...
              <mesh position={[0, 0.62, 0]} castShadow>
                ...
                <meshStandardMaterial color={i % 2 ? '#819166' : '#9da77e'} />
              </mesh>
            </group>
          );
        })}
